...
export const createUser = createAsyncThunk("user/createUser", async (formData) => {
  const response = await fetch("http://ancient-sands-09925.herokuapp.com/signup", {
    method: "POST",
    credentials: "include",
    body: formData
  });
...
export const deleteUser = createAsyncThunk("/user/deleteUser", async (id) => {
  const response = await fetch(`http://ancient-sands-09925.herokuapp.com/users/${id}`, { 
      method: "DELETE",
      credentials: "include"
 });
  const data = await response.json();
...
export const onLogin = createAsyncThunk("user/onLogin", async (user) => {
    const response = await fetch("http://ancient-sands-09925.herokuapp.com/login", {
        method: "POST", 
...
export const onLogout = createAsyncThunk("user/onLogout", async() => {
    const response = await fetch("http://ancient-sands-09925.herokuapp.com/logout", {
        method: "DELETE",
        credentials: "include"
    })
    const data = await response.json()
